...
function recognizeImageLexiconFree(model, criterion, image)
    --[[ Lexicon-free text recognition.
    ARGS:
      - `model`    : CRNN model
      - `criterion`: CRNN criterion
    ...
    local loss, prob = criterion:forward(output, pred, true)
    local str = label2str(pred)[1]
    local rawStr = label2str(predRaw, true)[1]
    return str, rawStr, prob
end


function recognizeImageWithLexicion(model, image, lexicon, topN)
    ...
    image = image:view(1, 1, image:size(1), image:size(2))
    local output = model:forward(image)
    local resTopN, probTopN = decodingWithLexicon(output, lexicon, topN)
    return resTopN, probTopN
end

function recognizeImageWithLexicionBK(model, image, lexicon, topN)
    --[[ Text recognition with a lexicon.
    ARGS:
      - `imagePath` : string, image path
      - `lexicon`   : list of string, lexicon words
    ...
    image = image:view(1, 1, image:size(1), image:size(2))
    local output = model:forward(image)
    local resTopN, probTopN = decodingWithLexiconBK(output, lexicon, topN)
    return resTopN, probTopN
end
