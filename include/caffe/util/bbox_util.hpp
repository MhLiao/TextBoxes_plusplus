...
typedef map<int, vector<NormalizedPolygon> > LabelPolygon;
...
float ray_length_in_bbox(float cx, float cy, float theta);
...
float RboxOverlap(const NormalizedRBox& bbox1, const NormalizedRBox& bbox2);
...
void EncodeBBoxPolygon(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool encode_variance_in_target, const NormalizedBBox& bbox, const NormalizedPolygon& polygon,
    NormalizedBBox* encode_bbox, NormalizedPolygon* encode_polygon);

void EncodeBBoxRBox(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool encode_variance_in_target, const NormalizedBBox& bbox, const NormalizedRBox& rbox,
    NormalizedBBox* encode_bbox, NormalizedRBox* encode_rbox);
...
void DecodeBBoxPolygon(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool variance_encoded_in_target, const bool clip_bbox,
    const NormalizedBBox& bbox, const NormalizedPolygon& polygon, NormalizedBBox* decode_bbox, NormalizedPolygon* decode_polygon);

void DecodeBBoxRBox(const NormalizedBBox& prior_bbox,
    const vector<float>& prior_variance, const CodeType code_type,
    const bool variance_encoded_in_target, const bool clip_bbox,
    const NormalizedBBox& bbox, const NormalizedRBox& rbox, NormalizedBBox* decode_bbox, NormalizedRBox* decode_rbox);
...
void DecodeBBoxesPolygons(const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const CodeType code_type, const bool variance_encoded_in_target,
...
void DecodeBBoxesRBoxes(const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const CodeType code_type, const bool variance_encoded_in_target,
...
void DecodeBBoxesAll(const vector<LabelBBox>& all_loc_pred, const vector<LabelPolygon>& all_loc_preds_polygon,
...
void DecodeBBoxesAll(const vector<LabelBBox>& all_loc_pred, const vector<LabelRBox>& all_loc_preds_rbox,
    const vector<NormalizedBBox>& prior_bboxes,
    const vector<vector<float> >& prior_variances,
    const int num, const bool share_location,
    const int num_loc_classes, const int background_label_id,
...
void GetDetectionResults(const Dtype* det_data, const int num_det,
      ...
      map<int, LabelBBox>* all_detections, map<int, LabelPolygon>* all_detections_polygon);

template <typename Dtype>
void GetDetectionResults(const Dtype* det_data, const int num_det,
      const int background_label_id,
      map<int, LabelBBox>* all_detections, map<int, LabelRBox>* all_detections_rbox);
