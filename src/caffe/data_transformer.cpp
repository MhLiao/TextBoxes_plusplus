  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX) {
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
        ...
        const NormalizedPolygon& polygon = anno.polygon();
        // Adjust bounding box annotation.
        NormalizedBBox resize_bbox = bbox;
        NormalizedPolygon resize_polygon = polygon;
        ...
        NormalizedPolygon proj_polygon;
        if (ProjectBBoxPolygon(crop_bbox, resize_bbox, resize_polygon, &proj_bbox, &proj_polygon)) {
          ...
          NormalizedPolygon* transformed_polygon = transformed_anno->mutable_polygon();
          transformed_polygon->CopyFrom(proj_polygon);
          if (do_mirror) {
          ...
          if (do_resize && param_.has_resize_param()) {
            ExtrapolateBBoxPolygon(param_.resize_param(), img_height, img_width,
                crop_bbox, transformed_bbox, transformed_polygon);
          }
        }
      }
      // Save for output.
...
void DataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
  ...
  // Expand the datum.
  NormalizedBBox expand_bbox;
  // NormalizedPolgon expand_polygon;
