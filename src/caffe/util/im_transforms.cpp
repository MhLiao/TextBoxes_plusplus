...
int first_point(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4){
  float xmin = std::min(x1, x2);
  xmin = std::min(xmin, x3);
  xmin = std::min(xmin, x4);
  float ymin = std::min(y1, y2);
  ymin = std::min(ymin, y3);
  ymin = std::min(ymin, y4);
  float xmax = std::max(x1, x2);
  xmax = std::max(xmax,x3);
  xmax = std::max(xmax,x4);
  float ymax = std::max(y1, y2);
  ymax = std::max(ymax, y3);
  ymax = std::max(ymax, y4);
  float dist1 = point_dist(x1, y1, xmin, ymin) + point_dist(x2, y2, xmax, ymin) + point_dist(x3, y3, xmax, ymax) + point_dist(x4, y4, xmin, ymax);
  float dist2 = point_dist(x2, y2, xmin, ymin) + point_dist(x3, y3, xmax, ymin) + point_dist(x4, y4, xmax, ymax) + point_dist(x1, y1, xmin, ymax);
  float dist3 = point_dist(x3, y3, xmin, ymin) + point_dist(x4, y4, xmax, ymin) + point_dist(x1, y1, xmax, ymax) + point_dist(x2, y2, xmin, ymax);
  float dist4 = point_dist(x4, y4, xmin, ymin) + point_dist(x1, y1, xmax, ymin) + point_dist(x2, y2, xmax, ymax) + point_dist(x3, y3, xmin, ymax);
...
void UpdateBBoxByResizePolicy(const ResizeParameter& param,
                              ...
                              NormalizedBBox* bbox, NormalizedPolygon* polygon) {
  ...
  float x1 = polygon->x1() * old_width;
  float y1 = polygon->y1() * old_height;
  float x2 = polygon->x2() * old_width;
  float y2 = polygon->y2() * old_height;
  float x3 = polygon->x3() * old_width;
  float y3 = polygon->y3() * old_height;
  float x4 = polygon->x4() * old_width;
  float y4 = polygon->y4() * old_height;

  float new_x1 = x1; float new_y1 = y1;
  float new_x2 = x2; float new_y2 = y2;
  float new_x3 = x3; float new_y3 = y3;
  float new_x4 = x4; float new_y4 = y4;

  int first_point_index=first_point(x1,y1,x2,y2,x3,y3,x4,y4);
  if(first_point_index==2){
    new_x1 = x2; new_y1 = y2;
    new_x2 = x3; new_y2 = y3;
    new_x3 = x4; new_y3 = y4;
    new_x4 = x1; new_y4 = y1;
  }
  if(first_point_index==3){
    new_x1 = x3; new_y1 = y3;
    new_x2 = x4; new_y2 = y4;
    new_x3 = x1; new_y3 = y1;
    new_x4 = x2; new_y4 = y2;
  }
  if(first_point_index==4){
    new_x1 = x4; new_y1 = y4;
    new_x2 = x1; new_y2 = y1;
    new_x3 = x2; new_y3 = y2;
    new_x4 = x3; new_y4 = y3;
  ...
  switch (param.resize_mode()) {
    case ResizeParameter_Resize_mode_WARP:
      ...
      new_x1 = new_x1 * new_width / old_width;
      new_y1 = new_y1 * new_height / old_height;
      new_x2 = new_x2 * new_width / old_width;
      new_y2 = new_y2 * new_height / old_height;
      new_x3 = new_x3 * new_width / old_width;
      new_y3 = new_y3 * new_height / old_height;
      new_x4 = new_x4 * new_width / old_width;
      new_y4 = new_y4 * new_height / old_height;
      break;
  ...
  new_x1 = new_x1 / new_width;
  new_y1 = new_y1 / new_height;
  new_x2 = new_x2 / new_width;
  new_y2 = new_y2 / new_height;
  new_x3 = new_x3 / new_width;
  new_y3 = new_y3 / new_height;
  new_x4 = new_x4 / new_width;
  new_y4 = new_y4 / new_height;

  polygon->set_x1(new_x1);
  polygon->set_y1(new_y1);
  polygon->set_x2(new_x2);
  polygon->set_y2(new_y2);
  polygon->set_x3(new_x3);
  polygon->set_y3(new_y3);
  polygon->set_x4(new_x4);
  polygon->set_y4(new_y4);
}
