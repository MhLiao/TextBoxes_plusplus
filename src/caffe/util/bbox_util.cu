...
__global__ void DecodeBBoxesKernel(const int nthreads,
          ...
          const bool clip_bbox, Dtype* bbox_data, bool use_polygon) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    if(use_polygon){
      const int i = index % 12;
      const int c = (index / 12) % num_loc_classes;
      const int d = (index / 12 / num_loc_classes) % num_priors;
      ...
      if (code_type == PriorBoxParameter_CodeType_CORNER) {
        ...
        const Dtype x1 = loc_data[index - i + 4];
        const Dtype y1 = loc_data[index - i + 5];
        const Dtype x2 = loc_data[index - i + 6];
        const Dtype y2 = loc_data[index - i + 7];
        const Dtype x3 = loc_data[index - i + 8];
        const Dtype y3 = loc_data[index - i + 9];
        const Dtype x4 = loc_data[index - i + 10];
        const Dtype y4 = loc_data[index - i + 11];

        Dtype decode_bbox_center_x, decode_bbox_center_y;
        Dtype decode_bbox_width, decode_bbox_height;
        Dtype decode_x1, decode_y1, decode_x2, decode_y2, decode_x3, decode_y3, decode_x4, decode_y4;
        if (variance_encoded_in_target) {
          ...
          decode_x1 = prior_data[vi] * x1 * prior_width + p_xmin;
          decode_y1 = prior_data[vi + 1] * y1 * prior_height + p_ymin;
          decode_x2 = prior_data[vi] * x2 * prior_width + p_xmax;
          decode_y2 = prior_data[vi + 1] * y2 * prior_height + p_ymin;
          decode_x3 = prior_data[vi] * x3 * prior_width + p_xmax;
          decode_y3 = prior_data[vi + 1] * y3 * prior_height + p_ymax;
          decode_x4 = prior_data[vi] * x4 * prior_width + p_xmin;
          decode_y4 = prior_data[vi + 1] * y4 * prior_height + p_ymax;
        }
        ...
        switch (i) {
          ...
          case 3:
            ...
            break;
          case 4:
            bbox_data[index] = decode_x1;
            break;
          case 5:
            bbox_data[index] = decode_y1;
            break;
          case 6:
            bbox_data[index] = decode_x2;
            break;
          case 7:
            bbox_data[index] = decode_y2;
            break;
          case 8:
            bbox_data[index] = decode_x3;
            break;
          case 9:
            bbox_data[index] = decode_y3;
            break;
          case 10:
            bbox_data[index] = decode_x4;
            break;
          case 11:
            bbox_data[index] = decode_y4;
            break;
        }
      }
      if (clip_bbox) {
      ...
      const int i = index % 9;
      const int c = (index / 9) % num_loc_classes;
      const int d = (index / 9 / num_loc_classes) % num_priors;
      if (!share_location && c == background_label_id) {
        // Ignore background class if not share_location.
        return;
      }
      ...
      if (code_type == PriorBoxParameter_CodeType_CORNER) {
        ...
        const Dtype x2 = loc_data[index - i + 6];
        const Dtype y2 = loc_data[index - i + 7];
        const Dtype h = loc_data[index - i + 8];

        Dtype decode_bbox_center_x, decode_bbox_center_y;
        Dtype decode_bbox_width, decode_bbox_height;
        Dtype decode_x1, decode_y1, decode_x2, decode_y2, decode_h;
        if (variance_encoded_in_target) {
          // variance is encoded in target, we simply need to retore the offset
          // predictions.
          decode_bbox_center_x = xmin * prior_width + prior_center_x;
          ...
          decode_x1 = prior_data[vi] * x1 * prior_width + prior_center_x;
          decode_y1 = prior_data[vi + 1] * y1 * prior_height + prior_center_y;
          decode_x2 = prior_data[vi] * x2 * prior_width + prior_center_x;
          decode_y2 = prior_data[vi + 1] * y2 * prior_height + prior_center_y;
          decode_h = exp(prior_data[vi + 3] * h) * prior_height;
        }
...
void DecodeBBoxesGPU(const int nthreads,
  ...
  DecodeBBoxesKernel<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      ...
      background_label_id, clip_bbox, bbox_data, use_polygon);
...
template void DecodeBBoxesGPU(const int nthreads,
          ...
          const int num_priors, const bool share_location,
          const int num_loc_classes, const int background_label_id,
          const bool clip_bbox, float* bbox_data, bool use_polygon);
template void DecodeBBoxesGPU(const int nthreads,
          const double* loc_data, const double* prior_data,
          const CodeType code_type, const bool variance_encoded_in_target,
          const int num_priors, const bool share_location,
          const int num_loc_classes, const int background_label_id,
          const bool clip_bbox, double* bbox_data, bool use_polygon);
