...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  use_polygon_ = multibox_loss_param_.use_polygon();
  ...
  if(use_polygon_){
    loc_shape.push_back(4 + 8);
  }else{
    loc_shape.push_back(4 + 5);
  }
...
void MultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if(use_polygon_){
    CHECK_EQ(num_priors_ * loc_classes_ * (4 + 8), bottom[0]->channels())
        << "Number of priors must match number of location predictions.";
  }else{
    CHECK_EQ(num_priors_ * loc_classes_ * (4 + 5), bottom[0]->channels())
        << "Number of priors must match number of location predictions.";
  }
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(use_polygon_){
    ...
    if (num_matches_ >= 1) {
      ...
      loc_shape[0] = 1;
      loc_shape[1] = num_matches_ * (4 + 8);
      loc_pred_.Reshape(loc_shape);
      loc_gt_.Reshape(loc_shape);
      Dtype* loc_pred_data = loc_pred_.mutable_cpu_data();
      Dtype* loc_gt_data = loc_gt_.mutable_cpu_data();
      EncodeLocPredictionBboxPolygon(all_loc_preds, all_loc_preds_polygon,
                          all_gt_bboxes, all_gt_polygons, all_match_indices_,
    ...
    if (this->layer_param_.propagate_down(0)) {
      ...
      float location_loss = loc_weight_ * loc_loss_.cpu_data()[0] / normalizer;
      if (location_loss>20){
        location_loss = 20.0;
      }
      top[0]->mutable_cpu_data()[0] += location_loss;
      // LOG(INFO)<<"loc loss:"<<location_loss;
    }
    ...
    if (num_matches_ >= 1) {
      ...
      vector<int> loc_shape(2);
      loc_shape[0] = 1;
      loc_shape[1] = num_matches_ * (4 + 5);
      loc_pred_.Reshape(loc_shape);
      loc_gt_.Reshape(loc_shape);
      Dtype* loc_pred_data = loc_pred_.mutable_cpu_data();
      Dtype* loc_gt_data = loc_gt_.mutable_cpu_data();
      EncodeLocPredictionBboxRbox(all_loc_preds, all_loc_preds_rbox,
                          all_gt_bboxes, all_gt_rboxes, all_match_indices_,
                          prior_bboxes, prior_variances, multibox_loss_param_,
                          loc_pred_data, loc_gt_data);
      loc_loss_layer_->Reshape(loc_bottom_vec_, loc_top_vec_);
      loc_loss_layer_->Forward(loc_bottom_vec_, loc_top_vec_);
      ...
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            if(use_polygon_){
              int start_idx = loc_classes_ * 12 * j + label * 12;
              caffe_copy<Dtype>(12, loc_pred_diff + count * 12,
                                loc_bottom_diff + start_idx);
            }else{
              int start_idx = loc_classes_ * 9 * j + label * 9;
              caffe_copy<Dtype>(9, loc_pred_diff + count * 9,
                                loc_bottom_diff + start_idx);
            }
